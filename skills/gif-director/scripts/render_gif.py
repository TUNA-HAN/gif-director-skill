...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--preset", default="caption-pop", choices=["caption-pop", "gentle-zoom", "shake", "bounce", "polaroid"])
...
def transform_frame(source, preset: str, width: int, height: int, t: float):
    if preset == "polaroid":
...
def main() -> int:
    ...
    for index in range(frame_count):
        ...
        frame = transform_frame(source, args.preset, args.width, args.height, t).convert("RGBA")
        ...
        draw_caption(frame, args.text, min(1, progress), zone="bottom")
        frames.append(frame.convert("RGB"))
    report = save_gif(frames, args.output, per_frame_ms)
    report.update({"preset": args.preset, "source": str(args.image), "ok": True})
